...
// From the auxiliary functions but since in specs said should be included in the class that implements it
...
void Game::play()
{
    ...
    while ( ! p->isPassedOut()  &&  m_city->flatulanCount() > 0)
    {
        ...
        {
            p->preach();
            m_city->moveFlatulans(); // all flatulans move after preaching
        }
        else
        {
            switch (action[0])
            {
                ...
                case 'h':
                    ...
                    cin.ignore(10000, '\n');
                    break; // nothing moves after checking history
                case 'u':
                case 'd':
                case 'l':
                case 'r':
                    p->move(decodeDirection(action[0]));
                    m_city->moveFlatulans(); // flatulans move after you move
                    break;
            }
        }
        m_city->display();
